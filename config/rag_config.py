...
class LangChainConfig:
    ...
    # Data settings
    mail_data_dir: str = "rag_service/data/message_by_thread"
    doc_data_dir: str = "rag_service/data/documentation"
    ...
    def validate(self) -> bool:
        ...
        if "mail" in self.base_retriever_types and not os.path.exists(
            self.mail_data_dir
        ):
            warnings.warn(
                f"mail_data_dir does not exist: {self.mail_data_dir}. "
                f"Email loading from files will be skipped. Using ChromaDB only."
            )
        if "documentation" in self.base_retriever_types and not os.path.exists(
            self.doc_data_dir
        ):
            warnings.warn(
                f"doc_data_dir does not exist: {self.doc_data_dir}. "
        ...
        for retriever_type in self.base_retriever_types:
            if retriever_type not in valid_types:
                raise ValueError(
                    f"Invalid base_retriever_type: {retriever_type}. Must be one of {valid_types}"
                )
        return True
