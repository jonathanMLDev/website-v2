...
class LLMHelper:
    ...
    def __init__(
        ...
        if agent:
            ...
            actual_model = config.openai_model if hasattr(config, 'openai_model') else "gpt-3.5-turbo"
    ...
    def _process_classify(self, document: Document) -> str:
        ...
        prompt, system_prompt = self.prompt_producer.produce_prompt('classify', document)
    ...
    def _process_sentiment(self, document: Document) -> str:
        ...
        prompt, system_prompt = self.prompt_producer.produce_prompt('sentiment', document)
        raw_result = self.agent.run_llm(prompt, system_prompt, max_tokens=200)
    ...
    def _process_libraries(self, document: Document) -> List[str]:
        ...
        prompt, system_prompt = self.prompt_producer.produce_prompt('extract_libraries', document)
    ...
    def _process_extract_all_meta(self, document: Document) -> Dict[str, Any]:
        ...
        prompt, system_prompt = self.prompt_producer.produce_prompt('extract_all_meta', document)
    ...
    def _process_extract_topics(self, documents: List[Document]) -> List[str]:
        ...
        if not documents:
            return []
        try:
            prompt, system_prompt = self.prompt_producer.produce_prompt('extract_topics', documents)
    ...
    def _process_summarize_chronologically(self, documents: List[Document], topic: str) -> str:
        ...
        try:
            ...
            result = self._postprocess_summarize_topic_chronologically(raw_result, topic)
            return result
        except Exception as e:
    ...
    def _process_total_summarize(self, documents: List[Document]) -> Dict[str, Any]:
        ...
        try:
            prompt, system_prompt = self.prompt_producer.produce_prompt('total_summarize', documents)
    ...
    def _postprocess_extract_all_meta(self, raw_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _parse_categories_from_meta(self, raw_result: Dict[str, Any]) -> List[str]:
        ...
        for entry in raw_categories:
            ...
            if label_str not in category_confidence or conf > category_confidence[label_str]:
                category_confidence[label_str] = conf
        
        categories = sorted(category_confidence, key=category_confidence.get, reverse=True)[:3]
        return categories if categories else ['Discussion']
    ...
    def _parse_sentiment_from_meta(self, raw_result: Dict[str, Any]) -> str:
        ...
        if isinstance(sentiment_data, dict):
            ...
            sentiment_confidence = sentiment_data.get('confidence', sentiment_data.get('score', 0.0))
        else:
            valid_sentiments = ['Positive', 'Negative', 'Neutral', 'Urgent']
            sentiment_label = (
                sentiment_data if sentiment_data in valid_sentiments
                else 'Neutral'
            )
    ...
    def _parse_libraries_from_meta(self, raw_result: Dict[str, Any]) -> Tuple[List[str], Dict[str, float]]:
        ...
        for entry in raw_libraries:
            ...
            if label_str == 'General':
                library_confidence['General'] = max(library_confidence.get('General', 0.0), conf)
                libraries.append('General')
                continue
            
            matched_name = self._match_library_name(label_str, known_libraries)
            if matched_name:
                library_confidence[matched_name] = max(library_confidence.get(matched_name, 0.0), conf)
    ...
    def _match_library_name(self, label_str: str, known_libraries: List[str]) -> Optional[str]:
    ...
    def _deduplicate_and_clean_libraries(self, libraries: List[str], library_confidence: Dict[str, float]) -> List[str]:
    ...
    def _postprocess_summarize_topic_chronologically(self, raw_result: Dict[str, Any], topic: str) -> str:
        """Postprocess chronological summarization result"""
        summary = raw_result.get('chronological_summary', f"Topic: {topic} - relevant discussions found.")
        return summary
    
    def _postprocess_total_summarize(self, raw_result: Dict[str, Any]) -> Dict[str, Any]:
        """Postprocess total summarization result"""
        if not raw_result:
            return {
                "subject": "No subject extracted",
                "assertions": []
            }
